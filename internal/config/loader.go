...
func WarnInsecureConfig(cfg *Config) {
	...
	if cfg.Auth.Enabled && cfg.Auth.SessionSecret != "" {
		...
		if strings.Contains(lower, "dev-") || strings.Contains(lower, "change") {
			slog.Warn("INSECURE CONFIG: session_secret appears to be a development placeholder; set a strong random value in production",
	...
	if cfg.Auth.Basic != nil {
		for _, user := range cfg.Auth.Basic.Users {
			if user.Password == user.Username {
				slog.Warn("INSECURE CONFIG: basic auth password equals username; use a strong password in production",
