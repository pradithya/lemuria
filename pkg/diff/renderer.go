...
// Returns true if the caller should return early (no diffs to render).
func (r *Renderer) renderNewAppPlan(sb *strings.Builder, result PlanResult) bool {
	if result.IsGeneratedApp {
		fmt.Fprintf(sb, "➕ **New application** - will be generated by ApplicationSet `%s` after merge\n\n", result.ApplicationSetName)
	} else {
...
// renderDeletedAppPlan writes the deleted-application-specific content to sb.
// Returns true if the caller should return early (no diffs to render).
func (r *Renderer) renderDeletedAppPlan(sb *strings.Builder, result PlanResult) bool {
	if result.IsGeneratedApp {
		fmt.Fprintf(sb, "➖ **Application will be removed** - ApplicationSet `%s` generator no longer produces this app\n\n", result.ApplicationSetName)
	} else {
